...
export default function Social({ type }: { type: "social" | "publication" }) {
  ...
  const LinkItem = ({ value }: { value: any }) => (
    <li key={value.id}>
      <a
        href={value.url}
        target="_blank"
  ...
  return (
    <>
      {/* Mobile view: seamlessly wrapped list */}
      <ul className="flex md:hidden items-center flex-wrap gap-x-5 gap-y-4 my-10">
        {filteredLinks.map((value) => (
          <LinkItem key={value.id} value={value} />
        ))}
      </ul>

      {/* Desktop view: distinct rows */}
      <div className="hidden md:flex flex-col gap-y-4 my-10">
        {rows.map((rowNum) => {
